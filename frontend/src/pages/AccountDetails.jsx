...
export default function AccountDetails() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5 sm:space-y-6">
      <button onClick={() => navigate(-1)} className="flex items-center space-x-2 text-gray-600 dark:text-gray-300 dark:text-gray-600 hover:text-gray-900 dark:text-white transition-colors text-sm sm:text-base">
        <ArrowLeft className="h-4 w-4" /><span>Back</span>
      </button>

      <div className="card p-5 sm:p-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-2">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">{account.accountHolderName}</h2>
        ...
        <div className="border-t pt-4">
          <div className="grid grid-cols-2 gap-3 sm:gap-4 text-xs sm:text-sm">
            <div><span className="text-gray-500 dark:text-gray-400 dark:text-gray-500">Type</span><p className="mt-0.5"><StatusBadge status={account.accountType} /></p></div>
            ...
            <div><span className="text-gray-500 dark:text-gray-400 dark:text-gray-500">Created</span><p className="font-medium mt-0.5">{new Date(account.createdAt).toLocaleString('en-IN')}</p></div>
          </div>
        </div>
