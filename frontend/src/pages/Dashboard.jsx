...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Welcome to Digital Bank</h1>
        <p className="text-gray-500 mt-1 text-sm sm:text-base">Manage accounts, transfers, and transactions</p>
      </div>
        ...
        {actions.map((action) => {
          ...
          return (
            <button key={action.path} onClick={() => navigate(action.path)}
              ...
              <p className="text-xs sm:text-sm text-gray-500 mt-0.5">{action.desc}</p>
            </button>
          );
        })}
      </div>
      ...
      <div className="card p-4 sm:p-6">
        <h2 className="text-base sm:text-lg font-semibold text-gray-900 mb-4">Look Up Account</h2>
    ...
    <div className="mt-6 border-t pt-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        <div className="bg-gray-50 rounded-xl p-4">
          ...
          <p className="font-mono font-semibold text-gray-900 text-sm sm:text-base">{account.accountNumber}</p>
        </div>
        <div className="bg-gray-50 rounded-xl p-4">
          <div className="flex items-center space-x-2 text-gray-500 mb-1">
      ...
      <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs sm:text-sm">
        <div><span className="text-gray-500">Type</span><p className="font-medium mt-0.5"><StatusBadge status={account.accountType} /></p></div>
        ...
        <div><span className="text-gray-500">Created</span><p className="font-medium mt-0.5">{new Date(account.createdAt).toLocaleDateString('en-IN')}</p></div>
      </div>
    </div>
  );
}