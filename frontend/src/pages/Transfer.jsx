    ...
    <div className="max-w-lg mx-auto space-y-6">
      <div className="card p-6 sm:p-8">
        <div className="flex items-center space-x-3 mb-6">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Transfer Money</h2>
        </div>
      ...
      {result && (
        <div className="card p-5 sm:p-6">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs sm:text-sm">
            <div><span className="text-gray-500">ID</span><p className="font-mono font-medium truncate">{result.id}</p></div>
            <div><span className="text-gray-500">Status</span><p><StatusBadge status={result.status} /></p></div>
            <div><span className="text-gray-500">From</span><p className="font-mono truncate">{result.senderAccountNumber}</p></div>
            <div><span className="text-gray-500">To</span><p className="font-mono truncate">{result.receiverAccountNumber}</p></div>
            <div><span className="text-gray-500">Amount</span><p className="font-bold text-lg">₹{Number(result.amount).toLocaleString('en-IN')}</p></div>
            <div><span className="text-gray-500">Ref</span><p className="font-mono text-xs truncate">{result.referenceNumber}</p></div>
          </div>
